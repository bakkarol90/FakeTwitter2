package AppUserDAO.impl;

import AppUser.AppUser;
import AppUserDAO.AbstractAppUserDao;
import AppUserDAO.AppUserDao;
...
public class MySQLUserDao extends AbstractAppUserDao implements AppUserDao {
    ...
    public HashSet<AppUser> getNotFollowedUsers(AppUser loggedUser) {
        ...
        Query allUsers = query.setParameter("followed", loggedUser.getFollowing());
        return new HashSet<>(query.getResultList());
    }

    @Override
    public HashSet<AppUser> getFollowers(AppUser loggedUser) {
        Query query = em.createQuery("select u.followers from AppUser u where u.id = :userId");
